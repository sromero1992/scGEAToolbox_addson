...
[g3, irows, jrows] = intersect(sce_tmp1.g, sce_tmp2.g, 'stable');
diff_exp_result = sc_deg(sce_tmp1.X(irows, :), sce_tmp2.X(jrows, :), g3);
idx_conf = diff_exp_result.p_val <= 0.05;
diff_exp_result = diff_exp_result(idx_conf, :);
g0_de = diff_exp_result.gene;

clear diff_exp_result comparison_result idx_conf irows jrows g3 sce_tmp1 sce_tmp2;
...
for iter = 1:num_iterations
    ...
    for ibatch = 1:nbatch
        ...
        for jbatch = ibatch + 1:nbatch
            ...
            [g3, irows, jrows] = intersect(sce1.g, sce2.g, 'stable');
            diff_exp_result = sc_deg(sce1.X(irows, :), sce2.X(jrows, :), g3);
            idx_conf = diff_exp_result.p_val <= 0.05;
            diff_exp_result = diff_exp_result(idx_conf, :);
    ...
    for itab = 1:nres
        for jtab = itab + 1:nres
            g1 = Tdv.Result{itab}.gene(1:100);
            g2 = Tdv.Result{jtab}.gene(1:100);
            g3 = Tde.Result{itab}.gene(1:100);
            g4 = Tde.Result{jtab}.gene(1:100);

            % Calculate the intersection and union dv
            overlap = intersect(g1, g2, 'stable');
            unionSet = union(g1, g2);

            % Calculate the intersection and union de
            overlap2 = intersect(g3, g4, 'stable');
            unionSet2 = union(g3, g4); 
            ...
            tab1_name = strcat('Table', string(itab));
            tab2_name = strcat('Table', string(jtab));
            Tovl_dv{it, 'BatchComparison'} = {strcat(tab1_name, '-', tab2_name)};
            Tovl_de{it, 'BatchComparison'} = {strcat(tab1_name, '-', tab2_name)};
