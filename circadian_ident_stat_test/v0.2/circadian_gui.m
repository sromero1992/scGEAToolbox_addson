function circadian_gui
    ...
    % Create UI controls
    uicontrol('Style', 'text', 'Position', [35, 550, 100, 20], 'String', 'Cell Type:'); % Adjusted position
    hCells = uicontrol('Style', 'popupmenu', 'Position', [120, 550, 150, 20], ... % Adjusted position
                       'String', cell_types, 'Value', 1); % Set default selection to the first option

    % Create dropdown for selecting plot_type
    uicontrol('Style', 'text', 'Position', [35, 500, 100, 20], 'String', 'Plot Type:'); % Adjusted position
    hPlotType = uicontrol('Style', 'popupmenu', 'Position', [120, 500, 170, 20], ... % Adjusted position
                          'String', plot_types);
    % Create checkbox for selecting period12
    hPeriod12 = uicontrol('Style', 'checkbox', 'Position', [65, 450, 210, 20], ... % Adjusted position
                          'String', 'Period 12 (otherwise 24)');

    % Heatmap Options
    uicontrol('Style', 'text', 'Position', [35, 400, 200, 20], 'String', 'Heatmap Options:'); % Added section header
    hStrictFilter = uicontrol('Style', 'checkbox', 'Position', [65, 370, 250, 20], ... % Added Strict filter checkbox
                              'String', 'Strict filtering (adj. p-value <= 0.05)', 'Value', 1); % Default to strict
    hClassicCirc = uicontrol('Style', 'checkbox', 'Position', [65, 340, 250, 20], ... % Added Classic circ genes checkbox
                             'String', 'Filter for classic circadian genes', 'Value', 0); % Default to not filter
    uicontrol('Style', 'text', 'Position', [35, 310, 120, 20], 'String', 'Custom Name:'); % Added Custom name label
    hCustomName = uicontrol('Style', 'edit', 'Position', [150, 310, 160, 25], ... % Added Custom name text field
    ...
    uicontrol('Style', 'pushbutton', 'Position', [50, 130, 150, 30], ... % Adjusted position
              'String', 'Plot single Gene', 'Callback', @plotGeneCallback);
    % Create checkbox for printing sc-data
    hPrintSCdata = uicontrol('Style', 'checkbox', 'Position', [65, 100, 230, 20], ... % Adjusted position
    ...
    function analyzeAndHeatmapCallback(~, ~)
        ...
        try
            ...
            if exist(analysis_fname, 'file')
                ...
                generateHeatmap_circ_simple(sce, celltype, strict, customName, circ);
