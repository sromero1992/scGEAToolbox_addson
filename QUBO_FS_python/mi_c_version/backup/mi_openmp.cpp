...
double compute_pairwise_mi(const std::vector<std::vector<double>>& matrix, int i, int j, int nbins) {
    ...
    // Parallelize joint histogram calculation
    ...
    // Normalize the joint counts to get the joint probabilities
    std::vector<std::vector<double>> joint_prob(nbins, std::vector<double>(nbins, 0.0));
    for (int m = 0; m < nbins; ++m) {
        for (int n = 0; n < nbins; ++n) {
            joint_prob[m][n] = joint_counts[m][n] / joint_sum;
        }
    }
...
int main() {
    ...
    double mi = compute_pairwise_mi(matrix, i, j, nbins);
